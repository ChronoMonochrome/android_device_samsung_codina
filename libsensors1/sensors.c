...
static int activate_acc(int enable)
{
	...
	if (enable) {
		...
		if (count_acc == 0) {
			write_cmd("/sys/class/input/input7/enable", "1",2);
			acc_thread_exit = 0;
		...
		if (count_acc == 0) {
			 ...
			 */
			acc_thread_exit = 1;
			write_cmd("/sys/class/input/input7/enable", "0",2);
		}
	}
	return ret;
}
...
static int activate_mag(int enable)
{
	...
	if (enable) {
		if (count_mag == 0) {
			write_cmd("/sys/class/input/input8/enable", "1",2);
			mag_thread_exit = 0;
			pthread_attr_init(&attr);
			/*-
			 * Create thread in detached state, so that we
		...
		if (count_mag == 0) {
			 ...
			 */
			mag_thread_exit = 1;
			write_cmd("/sys/class/input/input8/enable", "0",2);
		}
	}
	return ret;
}
...
static int poll_orientation(sensors_event_t *values)
{
	...
	mag_x = (data_mag[0] * YAS530_RESOLUTION);
	mag_y = (data_mag[1] * YAS530_RESOLUTION);
	if (mag_x == 0) {
		if (mag_y < 0)
	...
	acc_x = (float) (-data_acc[1]*20.0f);
	acc_x *= CONVERT_A;
	acc_y = (float) (data_acc[0]*20.0f);
	acc_y *= CONVERT_A;
	acc_z = (float) (data_acc[2]*20.0f);
	acc_z *= CONVERT_A;
...
static int poll_accelerometer(sensors_event_t *values)
{
	...
	values->acceleration.x = (float) (data[0]) * 16.6666666666666666f;
	values->acceleration.x *= CONVERT_A;
	values->acceleration.y = (float) (data[1]) * 16.6666666666666666f;
	values->acceleration.y *= CONVERT_A;
	values->acceleration.z = (float) (data[2]) * 16.6666666666666666f;
...
static int poll_magnetometer(sensors_event_t *values)
{
	...
	sscanf(buf, "%d", &raw_data);
	close(fd);
        //raw_data *= YAS530_RESOLUTION;
	data[0] = (raw_data >> 16) & 0xff;
	data[1] = (raw_data >> 8) & 0xff;
	data[2] = (raw_data) & 0xff;
	ALOGE("Mag data: %x (%d %d %d)", raw_data, data[0], data[1], data[2]);

	values->magnetic.status = SENSOR_STATUS_ACCURACY_HIGH;
	values->magnetic.x = (data[0]);
	values->magnetic.y = (data[1]);
	values->magnetic.z = (data[2]);
